...
// "layers" icon (Lucide), used for the ComfyUI menu/action-bar slots, whose
// `icon` field is a CSS *class* string (not markup) -- so we expose
// `BRP_ICON_CLASS`, a class we style with a mask-image of this SVG, tinted via
// currentColor. This frees us from PrimeIcons without adding a font, CDN, or
// build step. (The DOM-fallback button uses a solid "B" badge instead.)
const LAYERS_PATHS =
...
app.registerExtension({
  ...
  async setup() {
    ...
    try {
      ...
      btn.id = "brp-open-btn";
      // A solid "B" badge (à la the LoRA Manager button): far easier to spot and
      // identify than a monochrome outline icon that blends into the menu bar.
      btn.textContent = "B";
      btn.title = "Open the Batch Render UI";
      btn.setAttribute("aria-label", "Open the Batch Render UI");
      btn.style.cssText =
        "display:inline-flex;align-items:center;justify-content:center;cursor:pointer;" +
        "width:2rem;height:2rem;padding:0;margin:0 6px;border-radius:6px;" +
        "background:#6ea8fe;border:none;color:#10243f;" +
        "font:700 1.05rem/1 system-ui,-apple-system,Segoe UI,Roboto,sans-serif;";
